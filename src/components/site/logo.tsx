 ...
 * @file - logo.tsx
 *
 * The Scattered-Systems mark, rendered with CSS-variable fills so it tracks
 * the active theme (cyan glyph on an obsidian disc in dark, inverted in light).
 */
// imports
...
type LogoProps = Omit<
  ...
  'enableBackground' | 'role' | 'viewBox' | 'xmlns'
> & {
  background?: string;
  color?: string;
  opacity?: string;
...
/** The Scattered-Systems logo. */
export const ScsysLogo: React.FC<LogoProps> = ({
  ref,
  ...
  <svg
    ref={ref}
    className={cn(
      'size-8 rounded-full border border-primary/20 object-cover',
      className,
    )}
